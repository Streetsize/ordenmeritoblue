...
// CÁLCULO INTELIGENTE: Si ya cargó usamos su nota final. 
// Si no cargó, aplicamos estimación proporcional con examen + promedio estimativo (8.0) + bono Mendoza (+5)
function obtenerValorOrden(registro) {
   ...
   const promedioEstimado = 8.0; // Promedio base estándar estimativo
   const puntosExamen = notaEx * 0.90;
   const puntosPromedio = promedioEstimado * 0.5;
   const puntosMendoza = 5; // Asumimos el bono de Mendoza por defecto hasta que lo actualicen
...
function generarTabla(especialidadABuscar, dniSeleccionado = null) {
   ...
   competidores.forEach((c, index) => {
       ...
       let iconoEstado = c.NOTA_FINAL ? '✅' : '⏳ (Provisorio)';
